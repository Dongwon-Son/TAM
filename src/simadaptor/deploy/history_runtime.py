...
class RealTimeHistoryAdaptor:
    ...
    def from_checkpoint(
        ...
        **kwargs,
        ...
        released checkpoint (a key of ``simadaptor.assets.CHECKPOINTS``), or
        ``None`` for the default release (``SIMADAPTOR_DEFAULT_CHECKPOINT`` if
        set); released checkpoints are fetched once into ``~/.cache/simadaptor``.
        ``xml_path=None`` uses the packaged ideal-model MJCF. Remaining keyword
        arguments are forwarded to the regular constructor.
